...
| `src/hadith_mcp/assets/hadith_app.html`  | Self-contained MCP App UI served at `ui://hadith.html` for the `show_hadith` tool (interactive reader) |
...
**Interactive reader (`show_hadith`).** The tool binds to a `ui://hadith.html` resource served as `text/html;profile=mcp-app`. The HTML lives at `src/hadith_mcp/assets/hadith_app.html` (self-contained: inline CSS + ES module script). At load time the app imports the MCP App SDK from `https://esm.sh/@modelcontextprotocol/ext-apps` and fonts from Google Fonts; both domains are declared under the resource's `ui.csp.resourceDomains` so compliant hosts allow them. Once connected, the embedded app calls `fetch_hadith` and `search_hadith` over the MCP bridge (no extra HTTPS) to let users browse, open cross-references, and switch between detail and search views without LLM round-trips.
