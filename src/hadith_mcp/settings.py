...
def _resolve_db_path(doc: dict[str, Any], config_yaml: Path | None) -> Path:
    env = os.environ.get("HADITH_MCP_DB_PATH", "").strip()
    if env:
    ...
    return Path("data/hadith.db").expanduser().resolve()
...
def load_app_config(*, config_yaml: Path | None = None) -> AppConfig:
    """Load config: YAML overlay + environment overrides.

    ``HADITH_MCP_DB_PATH`` overrides database path. ``HADITH_MCP_QUERY_EMBEDDING_MODEL`` overrides
    the OpenAI model used to embed **search queries** (must output the same dimension as rows in
    ``hadith.db``, i.e. ``text-embedding-3-large`` / 3072 for databases built by this repo).

    ``HADITH_MCP_RATE_LIMIT_SEARCH_RPM``: max semantic searches per minute per client key
    (``0`` / ``off`` / empty after YAML = disable). ``HADITH_MCP_SEARCH_CACHE_MAX`` caps LRU entries
    (``0`` = disable cache).
    """
    doc: dict[str, Any] = {}
    ...
    qm = os.environ.get("HADITH_MCP_QUERY_EMBEDDING_MODEL", "").strip()
    query_embedding_model = qm or y["query_embedding_model"]

    rpm_env = os.environ.get("HADITH_MCP_RATE_LIMIT_SEARCH_RPM", "").strip().lower()
    ...
    cache_env = os.environ.get("HADITH_MCP_SEARCH_CACHE_MAX", "").strip()
    if cache_env:
