...
class GroundingState:
    """Track whether full grounding rules were already sent for a logical MCP session."""
            ...
            if prev is not None and not force_full:
                if nonce is not None and nonce != prev.nonce:
                    return {
                        ...
                        "nonce": prev.nonce,
                        "brief": (
                            "Unknown nonce for this session. Omit ``nonce`` to get the "
                            "short repeat message, or pass ``force_full=True`` for the full rules."
                        ),
                ...
                return {
                    ...
                    "nonce": prev.nonce,
                    "brief": (
                        "Grounding rules were already sent for this session; "
                        "reuse the earlier tool result instead of re-fetching."
                    ),
                    "repeat_suppressed": True,
                    "error": None,
                }
