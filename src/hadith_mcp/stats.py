"""Persistent usage counters in a separate SQLite file (not hadith.db)."""
...
def default_stats_db_path() -> Path:
    env = os.environ.get("HADITH_MCP_STATS_DB_PATH", "").strip()
    if env:
...
def stats_visitor_hash(raw_client_key: str, salt: str) -> str:
    """Stable pseudonymous id for deduplicating visitors (never store raw IPs in DB)."""
    return hashlib.sha256(f"{salt}:{raw_client_key}".encode("utf-8")).hexdigest()


class StatsTracker:
    """Thread-safe counters: events table + unique_visitors by hashed client key."""

    def __init__(self, db_path: Path | None = None, *, salt: str | None = None) -> None:
        self._path = db_path or default_stats_db_path()
        self._salt = (salt or os.environ.get("HADITH_MCP_STATS_SALT", "").strip() or "hadith-mcp-stats")
    ...
    def record(self, source: str, kind: str, visitor_raw: str | None) -> None:
        """source is 'mcp' or 'api'; kind is 'search' or 'lookup'."""
