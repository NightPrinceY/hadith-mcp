...
``script-src``). To make the ``show_hadith`` MCP app render reliably, we
...
into ``src/hadith_mcp/assets/ext-apps.bundle.mjs`` and inline it at
...
def fetch(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "hadith-mcp/fetch"})
...
def rewrite_exports_to_global(data: bytes) -> bytes:
    ...
    ``window.__hadithMcpSdk = { A: X, B: Y, ... };`` so the bundle can be
